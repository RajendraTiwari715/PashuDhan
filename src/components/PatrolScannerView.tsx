...
import type { PatrolScanResult, DecisionCase } from '../types';
import { Radio, ShieldAlert, CheckCircle2, AlertTriangle, MapPin, Search, RefreshCw } from 'lucide-react';


interface PatrolScannerViewProps {
  onScanResultProcessed?: (result: PatrolScanResult) => void;
}

export const PatrolScannerView: React.FC<PatrolScannerViewProps> = ({ onScanResultProcessed }) => {
  const [tagInput, setTagInput] = useState('TAG-1002');
  const [distanceInput, setDistanceInput] = useState<number>(1.2);
  const [isQuerying, setIsQuerying] = useState(false);
  const [scanResult, setScanResult] = useState<PatrolScanResult | null>(null);
  ...
  const handleExecutePatrolScan = () => {
    ...
    setTimeout(() => {
      ...
      let decisionCase: DecisionCase = 'CASE_A_SAFE_ON_PREMISES';
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">
              पेट्रोलिंग टीम हेतु सर्वर लाइव क्वेरी एवं ऑटो-डिसिजन लॉजिक इंजन
            </p>
          </div>
        </div>
      </div>

      {/* Control Scanner Form */}
      <div className="glass-panel p-6 rounded-3xl border border-slate-700 space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="sm:col-span-2">
            ...
            <div className="flex gap-2">
              ...
              <button
                ...
                disabled={isQuerying}
                className="bg-cyan-600 hover:bg-cyan-500 text-slate-950 font-bold px-5 py-2.5 rounded-xl text-xs flex items-center gap-1.5 transition-colors"
              >
                {isQuerying ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                <span>{isQuerying ? 'क्वेरी जारी...' : 'स्कैन एवं जांचें'}</span>
              </button>
            </div>
          </div>
          ...
          <div>
            ...
            >
              <option value={1.2}>1.2 मीटर (Standard RFID)</option>
              <option value={1.8}>1.8 मीटर (Long-Range Tag)</option>
              <option value={2.5}>2.5 मीटर (High Power Scan)</option>
            </select>
          </div>
        </div>

        {/* Quick Demo Test Buttons */}
        <div className="pt-2 border-t border-slate-800 flex items-center gap-2 flex-wrap text-xs text-slate-400">
          <span>परीक्षण केस चुनें:</span>
          <button
            onClick={() => { setTagInput('TAG-1001'); handleExecutePatrolScan(); }}
            className="px-2.5 py-1 rounded-lg bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 text-[11px] font-semibold"
          >
            Case A (Safe On-Premises)
          </button>
          <button
            onClick={() => { setTagInput('TAG-1002'); handleExecutePatrolScan(); }}
            className="px-2.5 py-1 rounded-lg bg-amber-500/20 text-amber-300 border border-amber-500/30 text-[11px] font-semibold"
          >
            Case B (Auto-Flag Offense)
          </button>
          <button
            onClick={() => { setTagInput('TAG-9999'); handleExecutePatrolScan(); }}
            className="px-2.5 py-1 rounded-lg bg-rose-500/20 text-rose-300 border border-rose-500/30 text-[11px] font-semibold"
          >
            Case C (Unowned Rescue Alert)
          </button>
        </div>
      </div>
        ...
        <div className="glass-panel p-6 rounded-3xl border border-slate-700 space-y-6 animate-fadeIn">
            ...
            <div>
              {scanResult.decisionCase === 'CASE_A_SAFE_ON_PREMISES' && (
                <span className="bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 text-xs px-3.5 py-1.5 rounded-full font-bold flex items-center gap-1.5">
                  ...
                  <span>Case A: स्वामित्‍व ऑन-प्रिमाइसेस (No Action)</span>
                </span>
              )}
              {scanResult.decisionCase === 'CASE_B_AUTO_FLAG_VIOLATOR' && (
                <span className="bg-amber-500/20 text-amber-300 border border-amber-500/40 text-xs px-3.5 py-1.5 rounded-full font-bold flex items-center gap-1.5 animate-pulse">
                  ...
                  <span>Case B: ऑटो-फ्लैग (Notice Issued)</span>
                </span>
              )}
              {scanResult.decisionCase === 'CASE_C_UNOWNED_STRAY_RESCUE' && (
                <span className="bg-rose-500/20 text-rose-300 border border-rose-500/40 text-xs px-3.5 py-1.5 rounded-full font-bold flex items-center gap-1.5 animate-bounce">
                  ...
                  <span>Case C: अनपेक्षित / आवारा (Rescue Team Dispatched)</span>
                </span>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="w-full h-44 rounded-2xl overflow-hidden border border-slate-700 relative">
              <img src={scanResult.evidencePhotos[0]} alt="Evidence" className="w-full h-full object-cover" />
              <div className="absolute bottom-2 left-2 bg-slate-950/80 px-2 py-0.5 rounded text-[10px] text-cyan-300 font-mono">
                1-2m Scan Distance: {scanResult.scanDistanceMeters}m
              </div>
            </div>
